...
players. Game validation still applies. Every step emits a Frame, including empty
steps, so fixed tick progression is replayable. Frame checksums describe state
*after* applying that tick. Sequence is the count of accepted game commands in
this example, not the frame count or a future persistence journal sequence.
